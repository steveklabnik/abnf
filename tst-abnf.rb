...
class TestABNF < Test::Unit::TestCase
  def test_non_recursion_1
    assert_equal(/[Aa]/, ABNF.ruby_regexp(<<-'End').regexp)
      n = "a"
    End
  end

  def test_left_recursion_1
    assert_equal(/[Aa][Bb]*/, ABNF.ruby_regexp(<<-'End').regexp)
      n = "a" | n "b"
    End
  end
  ...
  def test_left_recursion_3
    assert_equal(/[Aa][Cc](?:[Bb][Cc])*/, ABNF.ruby_regexp(<<-'End').regexp)
      n = ("a" | n "b") "c"
    End
  end

  def test_right_recursion_1
    assert_equal(/[Bb]*[Aa]/, ABNF.ruby_regexp(<<-'End').regexp)
      n = "a" | "b" n
    End
  end

  def test_both_recursion_1
    assert_equal(/[Aa]*[Bb][Cc]*/, ABNF.ruby_regexp(<<-'End').regexp)
      n = "a" n | "b" | n "c"
    End
  end
